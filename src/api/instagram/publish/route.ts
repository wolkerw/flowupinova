...
async function createMediaContainer(
  ...
  const url = `https://graph.instagram.com/v20.0/${instagramId}/media`;
...
async function publishMediaContainer(
  ...
  const url = `https://graph.instagram.com/v20.0/${instagramId}/media_publish`;
  const params = new URLSearchParams({
  ...
  if (!response.ok || !data.id) {
    ...
    throw new Error(
      data.error?.message || "A API não retornou um ID de mídia publicado após a finalização."
    );
  }
  ...
  try {
    ...
    if (
      ...
      return NextResponse.json(
        {
          success: false,
          error: "Dados da requisição incompletos. Faltando postData ou detalhes da conexão Meta.",
        },
        { status: 400 }
      );
    }
    ...
    console.log(
      `[INSTAGRAM_PUBLISH_SUCCESS] Mídia publicada com sucesso no Instagram. Post ID: ${publishedMediaId}`
    );
