...
import { ArrowRight, Image as ImageIcon, Copy, Film, Sparkles, ArrowLeft, Video, FileImage, CheckCircle, ChevronLeft, ChevronRight, X, Loader2, Send, Calendar as CalendarIcon, Clock, AlertTriangle, Instagram, UploadCloud, Trash2 } from "lucide-react";
...
export default function CriarConteudoV2Page() {
    ...
    const handleSubmit = async () => {
        if (!user || !instagramConnection?.isConnected || mediaItems.length === 0) {
            toast({ variant: "destructive", title: "Erro", description: "Verifique se sua conta do Instagram está conectada e se você adicionou uma mídia." });
            return;
        }
        ...
        const result = await schedulePost(user.uid, {
            ...
            platforms: ['instagram'], // Only Instagram for V2
    ...
    return (
        <div className="p-6 space-y-8 max-w-7xl mx-auto">
            <div className="text-center">
                <h1 className="text-3xl font-bold text-gray-900">Criar Novo Conteúdo (V2)</h1>
                <p className="text-gray-600 mt-1">
                    ...
                    {step === 3 && `Etapa 3 de 3: Revise e agende sua publicação no Instagram`}
                </p>
            </div>
                                    ...
                                    <div className="w-full bg-white rounded-md shadow-lg border flex flex-col">
                                        ...
                                        <div className="p-3 text-sm min-h-[6rem]">
                                            <p className="whitespace-pre-wrap">
                                                <span className="font-bold">{instagramConnection?.instagramUsername || 'seu_usuario'}</span> {title && <span className="font-bold">{title}</span>} {text}
                                            </p>
                                        </div>
                                    </div>
                                </div>
                ...
                >
                    ...
                    <Card className="shadow-lg border-none">
                        ...
                        <CardFooter className="flex-col items-stretch">
                            ...
                            {!instagramConnection?.isConnected && (
                                <p className="text-xs text-red-600 mt-2 text-center flex items-center justify-center gap-1"><AlertTriangle className="w-4 h-4" /> Conecte sua conta do Instagram na página de Conteúdo V2 para publicar.</p>
                            )}
                        </CardFooter>
                    </Card>
