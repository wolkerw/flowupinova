  ...
  try {
    ...
    if (!accessToken || !accountId || !locationId) {
      return NextResponse.json(
        { success: false, error: "Access Token, Account ID e Location ID são obrigatórios." },
        { status: 400 }
      );
    }
    ...
    if (!response.ok) {
      ...
      if (response.status === 401 || response.status === 403) {
        console.warn(
          `[GOOGLE_MEDIA_AUTH_ERROR] Token inválido ou expirado. Status: ${response.status}`
        );
        return NextResponse.json(
      ...
      console.error(
        `[GOOGLE_MEDIA_ERROR] API retornou status ${response.status}. Resposta:`,
        errorText
      );
    ...
    const gallery = items.filter(
      (m: any) => !["COVER", "PROFILE"].includes(m.locationAssociation?.category ?? "")
    );

    const formattedMedia = {
      coverPhoto: cover
        ? { url: cover.googleUrl, thumbnailUrl: cover.thumbnailUrl, name: cover.name }
        : null,
      profilePhoto: profile
        ? { url: profile.googleUrl, thumbnailUrl: profile.thumbnailUrl, name: profile.name }
        : null,
    ...
    return NextResponse.json(
      { success: false, error: error.message || "Falha ao buscar mídias." },
      { status: 500 }
    );
  }
}
