...
export async function POST(request: NextRequest) {
  try {
    const { accessToken, locationId, startDate, endDate } =
      (await request.json()) as InsightsRequestBody;

    if (!accessToken || !locationId) {
      return NextResponse.json(
        { success: false, error: "Access Token e Location ID são obrigatórios." },
        { status: 400 }
      );
    }
    ...
    if (!insightsResponse.ok) {
      ...
      if (insightsResponse.status === 401 || insightsResponse.status === 403) {
        console.warn(
          `[GOOGLE_INSIGHTS_AUTH_ERROR] Token inválido ou expirado. Status: ${insightsResponse.status}`
        );
        return NextResponse.json(
          { success: false, error: "Token de acesso do Google inválido ou expirado." },
          { status: 401 }
        );
      }
      console.error("[GOOGLE_INSIGHTS_ERROR] API Response:", errorData);
      throw new Error(
        errorData.error?.message || "Falha ao buscar dados de performance do Google."
      );
    }
    ...
    insightsData.multiDailyMetricTimeSeries?.forEach((series: any) => {
      series.dailyMetricTimeSeries?.forEach((metricSeries: any) => {
        ...
        const totalValue =
          metricSeries.timeSeries?.datedValues?.reduce(
            (acc: number, curr: any) => acc + parseInt(curr.value || "0", 10),
            0
          ) || 0;
    ...
    const finalInsights = {
      viewsSearch:
        (aggregatedMetrics["BUSINESS_IMPRESSIONS_DESKTOP_SEARCH"] || 0) +
        (aggregatedMetrics["BUSINESS_IMPRESSIONS_MOBILE_SEARCH"] || 0),
      viewsMaps:
        (aggregatedMetrics["BUSINESS_IMPRESSIONS_DESKTOP_MAPS"] || 0) +
        (aggregatedMetrics["BUSINESS_IMPRESSIONS_MOBILE_MAPS"] || 0),
    ...
    if (profileResponse.ok) {
      ...
      console.warn(
        `[GOOGLE_INSIGHTS_WARN] Falha ao buscar dados de perfil para ${locationId}. Status: ${profileResponse.status}`
      );
