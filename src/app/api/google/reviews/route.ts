  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 401 || response.status === 403) {
        console.warn(
          `[GOOGLE_REVIEWS_AUTH_ERROR] Token inválido ou expirado. Status: ${response.status}`
        );
        return NextResponse.json(
      ...
      console.error(
        `[GOOGLE_REVIEWS_ERROR] API retornou status ${response.status}. Resposta:`,
        errorText
      );
    ...
    return NextResponse.json(
      { success: false, error: error.message || "Falha ao buscar avaliações." },
      { status: 500 }
    );
  }
}
