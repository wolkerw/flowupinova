...
export async function POST(request: NextRequest) {
  try {
      ...
      try {
        ...
        if (specificViolation) {
          ...
          if (generalMessage.toLowerCase().includes("invalid argument")) {
            errorMessage =
              "Não foi possível enviar a imagem. Verifique se o arquivo atende aos requisitos de uma foto de capa: formato JPG ou PNG e dimensões adequadas (proporção 16:9, ex.: 1280x720).";
          } else {
    ...
    console.error("[GOOGLE_UPLOAD_COVER_ERROR]", error);
