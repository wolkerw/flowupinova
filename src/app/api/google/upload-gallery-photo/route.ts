...
export async function POST(request: NextRequest) {
    try {
            ...
            try {
                ...
                if (specificViolation) {
                    ...
                    if (generalMessage.toLowerCase().includes("invalid argument")) {
                         errorMessage = "Não foi possível enviar a foto. Verifique se o arquivo atende aos requisitos: formato JPG ou PNG e dimensões mínimas de 250x250 pixels.";
                    } else {
        ...
        console.error("[GOOGLE_UPLOAD_GALLERY_ERROR]", error);
