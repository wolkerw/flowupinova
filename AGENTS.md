...
4. **REGRA INVIOLÁVEL: NUNCA SUBIR AUTOMATICAMENTE (PUSH SÓ QUANDO SOLICITADO)**: Nenhum assistente ou agente de IA tem autorização para executar `git push` (seja para `uat`, `main` ou qualquer branch) automaticamente, mesmo com 'Always Proceed' ativado. O agente DEVE SEMPRE manter as alterações localmente e **SÓ realizar `git push` quando o usuário pedir explicitamente** na conversa.
...
6. **Versionamento por Tags e Rollback Seguro**:
   - Cada release para a branch `main` deve receber uma **Git Tag semântica** (ex: `v1.0.0`, `v1.1.0`, etc.) enviada junto com o push (`git push origin main --tags`).
   - Quando o usuário solicitar **"faça rollback em main"** ou **"reverta para a versão anterior"**, o agente deve reverter a branch `main` para o commit da tag anterior estável e disparar o rollout correspondente no Firebase, liberando a branch `uat` para ajustes e novo deploy com nova tag.
