...
test.describe('Smoke Tests - Navegação Padrão', () => {
  ...
  test.describe('Páginas Autenticadas', () => {
    ...
    test('Acesso a Relatórios', async ({ page }) => {
      ...
      await expect(page.getByRole('heading', { name: /Relatórios/i }).first()).toBeVisible({ timeout: 15000 });
    });

    test('Acesso a Marketplace', async ({ page }) => {
      await page.goto('/dashboard/marketplace');
      await expect(page.getByRole('heading', { name: /Marketplace/i }).first()).toBeVisible({ timeout: 15000 });
    });
  });
});
